...
import { 
  ...
  Workflow, MessageSquare, FastForward, Sliders, Layers, Bot 
} from 'lucide-react';
    ...
    <div className="w-full bg-white font-sans overflow-hidden">
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-xl font-semibold text-gray-900 mb-8">AI-powered customer support automation platform</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 text-sm text-gray-600 font-medium">
            <div className="flex items-center gap-3 justify-center md:justify-start lg:justify-center p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
              <Globe className={`w-5 h-5 ${primaryColor}`} />
              <span className="text-left">Support omnichannel conversations across Web, WhatsApp, Email, and Voice</span>
            </div>
            <div className="flex items-center gap-3 justify-center md:justify-start lg:justify-center p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
              <Users className={`w-5 h-5 ${primaryColor}`} />
              <span className="text-left">Configurable escalation rules for smooth human handoff</span>
            </div>
            <div className="flex items-center gap-3 justify-center md:justify-start lg:justify-center p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
              <Settings className={`w-5 h-5 ${primaryColor}`} />
              <span className="text-left">Built to support multiple AI model providers (OpenAI, Anthropic, Gemini)</span>
            </div>
            <div className="flex items-center gap-3 justify-center md:justify-start lg:justify-center p-4 bg-white rounded-lg border border-gray-100 shadow-sm">
              <Shield className={`w-5 h-5 ${primaryColor}`} />
              <span className="text-left">Designed with security, privacy, and compliance in mind</span>
            </div>
          </div>
        </div>
      </motion.section>
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-12 text-left">
          ...
          <motion.div variants={fadeInUp} className="bg-[#f8f9fa] p-6 rounded-xl border border-gray-100">
            <span className="font-medium text-gray-800">Platforms that ignore established human workflows</span>
          </motion.div>
          <motion.div variants={fadeInUp} className="bg-[#f8f9fa] p-6 rounded-xl border border-gray-100">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <motion.div variants={fadeInUp} className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
              ...
              <p className="text-gray-600 text-sm leading-relaxed">Either you provide instant answers or ensure safe and accurate resolution through grounded knowledge.</p>
            </motion.div>
            <motion.div variants={fadeInUp} className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
              <h4 className={`text-xl font-bold ${primaryColor} mb-4`}>AI or humans</h4>
              <p className="text-gray-600 text-sm leading-relaxed">AI fails to route complex queries to specialized teams, creating confusion and delaying response times.</p>
            </motion.div>
            <motion.div variants={fadeInUp} className="bg-white p-8 rounded-2xl border border-gray-100 shadow-sm">
              <h4 className={`text-xl font-bold ${primaryColor} mb-4`}>Automation or CX</h4>
              <p className="text-gray-600 text-sm leading-relaxed">Deploying fully automated systems without human oversight often compromises long-term customer experience.</p>
            </motion.div>
          </div>
        </div>
      </motion.section>
      ...
      >
        <motion.h2 variants={fadeInUp} className={`text-2xl md:text-3xl font-bold ${primaryColor} mb-16`}>
          Kas newerax combines AI models, company knowledge, and conversation context.
        </motion.h2>
          ...
          <div className="flex flex-col lg:flex-row items-center gap-16">
               ...
               <div className="w-full aspect-video bg-white rounded-xl border border-gray-200 shadow-sm flex items-center justify-center relative overflow-hidden">
                  ...
                  <div className="p-8 w-full flex flex-col gap-4 mt-8">
                     <div className="bg-indigo-50 p-4 rounded-lg w-[80%]">
                        <p className="text-xs text-gray-500 font-medium mb-1 flex items-center gap-1"><Bot className="w-3 h-3" /> AI Assistant</p>
                        <p className="text-sm text-gray-800">Handling routine request based on knowledge base...</p>
                     </div>
                     <div className="bg-gray-50 p-4 rounded-lg w-[80%] self-end border border-gray-200">
                        <p className="text-xs text-gray-500 font-medium mb-1 flex items-center gap-1"><Users className="w-3 h-3" /> Human Support</p>
                        <p className="text-sm text-gray-800">Escalation triggered. Taking over context smoothly. Hi, I can help you with that complex issue.</p>
                     </div>
                  </div>
               </div>
            </motion.div>
            ...
            <motion.div variants={fadeInUp} className="w-full lg:w-1/2">
              ...
              <ul className="space-y-4">
                {[
                  "Immediate reduction in repetitive support workload",
                  "Clear visibility into AI agent behavior and conversation flow",
                  "Seamless human handoff within established customer support workflows",
                  "Gradual, controllable automation grounded in company knowledge"
                ].map((item, idx) => (
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
          <div className="w-full lg:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-8">
            <motion.div variants={fadeInUp} className="space-y-3">
              ...
              <h4 className="text-lg font-bold text-gray-900">Deploy Logically</h4>
              <p className="text-sm text-gray-600 leading-relaxed">Train your AI agent using your website, help docs, or FAQs. Deploy across multiple channels without rebuilding for each.</p>
            </motion.div>
            <motion.div variants={fadeInUp} className="space-y-3">
              <Settings className={`w-8 h-8 ${primaryColor} mb-2`} />
              <h4 className="text-lg font-bold text-gray-900">Automate the safe stuff</h4>
              <p className="text-sm text-gray-600 leading-relaxed">Handle repetitive, routine questions instantly. AI answers confidently where it's grounded in your data - and stops where it's not.</p>
            </motion.div>
            <motion.div variants={fadeInUp} className="space-y-3">
              <Users className={`w-8 h-8 ${primaryColor} mb-2`} />
              <h4 className="text-lg font-bold text-gray-900">Handoff when it matters</h4>
              <p className="text-sm text-gray-600 leading-relaxed">Complex, emotional, or sensitive conversations move smoothly to humans, with full context intact from the AI agent.</p>
            </motion.div>
            <motion.div variants={fadeInUp} className="space-y-3">
              <Sliders className={`w-8 h-8 ${primaryColor} mb-2`} />
              <h4 className="text-lg font-bold text-gray-900">Improve with confidence</h4>
              <p className="text-sm text-gray-600 leading-relaxed">See what AI handles, where it escalates, and how customers respond - keeping humans in control of the automation.</p>
            </motion.div>
          </div>
             ...
             <div className="w-full bg-[#f4f5f8] rounded-xl border border-gray-200 flex overflow-hidden min-h-[400px]">
                ...
                <div className="flex-1 p-6">
                  ...
                  <div className="space-y-4">
                     <div className="h-24 bg-white border border-gray-200 rounded w-full flex items-center px-4"><div className="w-10 h-10 bg-gray-100 rounded-full mr-4"></div><div className="flex-1 space-y-2"><div className="h-2 w-1/2 bg-gray-200 rounded"></div><div className="h-2 w-3/4 bg-gray-100 rounded"></div></div></div>
                     <div className="h-24 bg-white border border-gray-200 rounded w-full flex items-center px-4"><div className="w-10 h-10 bg-gray-100 rounded-full mr-4"></div><div className="flex-1 space-y-2"><div className="h-2 w-1/3 bg-gray-200 rounded"></div><div className="h-2 w-2/3 bg-gray-100 rounded"></div></div></div>
                     <div className="h-24 bg-white border border-gray-200 rounded w-full flex items-center px-4"><div className="w-10 h-10 bg-gray-100 rounded-full mr-4"></div><div className="flex-1 space-y-2"><div className="h-2 w-1/4 bg-gray-200 rounded"></div><div className="h-2 w-1/2 bg-gray-100 rounded"></div></div></div>
                  </div>
                </div>
             </div>
          </motion.div>
        ...
        <div className="flex flex-col lg:flex-row gap-16">
          <motion.div variants={fadeInUp} className="w-full lg:w-1/3 lg:sticky lg:top-24 h-fit">
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/signup" className={`px-6 py-3 rounded-md ${bgPrimary} text-white font-semibold text-center hover:bg-[#502cd1] transition-colors shadow-sm`}>
                Request Early Access
              </Link>
              <Link to="/book-demo" className={`px-6 py-3 rounded-md border border-[#6039ea] ${primaryColor} font-semibold text-center hover:bg-indigo-50 transition-colors`}>
                Talk to Sales
              </Link>
            </div>
          </motion.div>
          ...
          <motion.div variants={fadeInUp} className="w-full lg:w-2/3 relative">
            ...
            <div className="space-y-12">
              {[
                { icon: Workflow, title: "Structured Implementation", desc: "Integrates with your existing support stack without requiring a full rip-and-replace." },
                { icon: Users, title: "Human + AI by design", desc: "Handoff is native, not an afterthought in customer support." },
                { icon: Shield, title: "Control-first AI", desc: "Rules, routing, and visibility - keeping humans in control of escalation." },
                { icon: Layers, title: "Provider Flexibility", desc: "Built to support multiple AI models (OpenAI, Anthropic, Gemini) depending on your needs." },
                { icon: Globe, title: "Unified Automation", desc: "Manage automation rules across web, WhatsApp, and email from a central platform." }
              ].map((item, idx) => (
